				...
				highlightElement: function (e, t, n) {
					...
					j.hooks.run('before-sanity-check', i),
						...
						i.code
							...
							: (j.hooks.run('complete', i), n && n.call(i.element));
				},
												...
												for (
													...
													if (!(F instanceof C)) {
														var v,
															x = 1;
														if (m) {
															if (!(v = L(f, y, t, p)) || v.index >= t.length) break;
															var k = v.index,
															...
															for (
																...
																x++, (A += P.value.length);
															x--, (F = t.slice(y, A)), (v.index -= y);
														} else if (!(v = L(f, 0, F, p))) continue;
														var k = v.index,
															$ = v[0],
															S = F.slice(0, k),
															E = F.slice(k + $.length),
															F = y + F.length,
															_ = (i && F > i.reach && (i.reach = F), b.prev),
															S =
		...
		return (
			...
			l.document
				? ((a = j.util.currentScript()) &&
						((j.filename = a.src), a.hasAttribute('data-manual')) &&
						(j.manual = !0),
					j.manual ||
						('loading' === (e = document.readyState) || ('interactive' === e && a && a.defer)
